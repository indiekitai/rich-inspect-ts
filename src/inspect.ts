...
const style = {
  ...
  bgGray: (s: string) => `${ESC}48;5;236m${s}${RESET}`,
};
...
function getTypeName(obj: unknown): string {
  ...
  if (typeof obj === 'function') {
    ...
    return `${kind} ${(obj as Function).name || '<anonymous>'}`;
  }
...
function getPrototypeChain(obj: unknown): string {
...
function safeStringify(val: unknown, depth = 2): string {
  try {
    if (val === undefined) return 'undefined';
    if (val === null) return 'null';
    if (typeof val === 'function') {
      const src = val.toString();
      if (src.length > 120) return src.slice(0, 117) + '...';
      return src;
    }
    ...
    if (ArrayBuffer.isView(val)) return `${val.constructor.name}(${(val as any).length})`;
...
function getDoc(obj: unknown): string | undefined {
  ...
  if (obj && typeof obj === 'object') {
    const desc = (obj as any).description ?? (obj as any).doc ?? (obj as any).__doc__;
...
function collectMembers(
  obj: unknown,
  opts: Required<Pick<InspectOptions, 'methods' | 'private' | 'symbols' | 'sort' | 'docs' | 'help'>>
): { members: MemberInfo[]; hiddenCount: number } {
  if (obj === null || obj === undefined) return { members: [], hiddenCount: 0 };
  ...
  if (opts.symbols) {
    try {
      for (const sym of Object.getOwnPropertySymbols(obj)) {
        if (!seen.has(sym)) { seen.add(sym); allKeys.push(sym); }
      }
    } catch { /* empty */ }
  }
  ...
  const filteredKeys = allKeys.filter(key => {
    if (typeof key === 'symbol') return opts.symbols;
    if (key === 'constructor') return false;
    if (key.startsWith('__') && key.endsWith('__')) return opts.private; // dunder-like
    if (key.startsWith('_')) return opts.private;
    return true;
  });
  ...
  for (const key of filteredKeys) {
    ...
    if (typeof val === 'function') {
      ...
      if (opts.docs) {
        doc = getDoc(val);
        if (doc && !opts.help) doc = firstParagraph(doc);
      }
      members.push({
        name: keyName,
        kind: 'method',
 ...
 */
export function inspectObject(obj: unknown, options: InspectOptions = {}): InspectResult {
  const {
    title,
    help = false,
    methods = false,
  ...
  if (docs || help) {
    doc = getDoc(obj);
    if (doc && !help) doc = firstParagraph(doc);
  }
...
function boxLine(content: string, width: number, position: 'top' | 'mid' | 'bot', title?: string): string {
...
function highlightValue(val: unknown): string {
  const s = safeStringify(val);
  if (val === null || val === undefined) return style.italic(style.magenta(s));
  if (typeof val === 'string') return style.green(`"${val}"`);
  if (typeof val === 'number' || typeof val === 'bigint') return style.cyan(s);
  if (typeof val === 'boolean') return style.yellow(s);
  return s;
}
  ...
  if (result.members.length > 0) {
    ...
    for (const m of result.members) {
      ...
      if (m.kind === 'method') {
        ...
        const nameStyle = m.name.startsWith('_') ? style.dim : (s: string) => s;
